...
| 13 | Config loading & precedence | `IngestMain.scala:93-120` | `IngestMain.loadBaseConfig` | `--conf-path` else `ConfigFactory.load()`; feed = `feeds.<entity>` | `--conf-path` (explicit file) beats `-Dconfig.file` beats classpath `application.conf` (Typesafe Config standard). **Updated 2026-08-06:** both routes are absolutised so HOCON `include`s resolve from a bare filename; a missing file raises CFG_018 instead of silently loading an empty config. Covered by `ConfigLoadIncludeSpec`. **Updated 2026-08-16:** `--override-path` adds an operational layer that WINS over the feed for every path it declares (merged before `resolve()`); overridden paths are logged by name only, never values, and the ledger's `config_fingerprint` gains an `+ovr:<digest>` suffix. Missing override file = CFG_019. Covered by `ConfigOverrideSpec`. |
| 14 | Deployment script | `scripts/run_health_sherpa.sh` | — | `--files "$CONF_FILE"` (+ `INGEST_EXTRA_FILES` for split configs), `--conf-path ./<basename>`, `--entity health_sherpa_member` | Delegates to `scripts/ingest_submit_common.sh`. No credentials on the command line. **Updated 2026-08-06:** was `-Dconfig.file=` on driver *and* executors; that could not resolve HOCON `include`s (bare filename has no parent directory) and the executor copy was dead weight — only the driver reads the feed config. |
