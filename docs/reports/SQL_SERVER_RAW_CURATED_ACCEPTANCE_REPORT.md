...
|---|---|---|---|---|
| P0 baseline & plan | **Met** | `SQL_SERVER_RAW_CURATED_IMPLEMENTATION_PLAN.md`, `REQUIREMENTS_COMPLIANCE_STATUS.md` | n/a (docs) | Gap analysis re-verified against branch before any change |
...
| P7 run locking | **Met** (best-effort store = **Accepted deviation**) | `LockService` (lease/renewal/expiry pre-existing; `wait_ms` added) | `LockServiceSpec` (pre-existing), integration "wait_ms retries a held lock" | Hive append lease is documented best-effort; watermark version CAS remains defense-in-depth; a transactional lock store is the recorded follow-up (a reference `JdbcCheckpointStore` pattern survives in git history at v1.0.0-snapshot.6) |
